...
def create_simple_terminal():
    """Create simple terminal display without </div> issue"""
    terminal_content = st.session_state.terminal_log
    
    # Use proper HTML escaping to avoid </div> showing up
    terminal_content_escaped = terminal_content.replace('<', '&lt;').replace('>', '&gt;')
...
{terminal_content_escaped}
    </div>
    """, unsafe_allow_html=True)

def create_terminal_with_sync():
    """Create terminal with typing effect and audio sync"""
    import base64
    import streamlit.components.v1 as components
    
    # Handle typing effect or normal display
    if st.session_state.show_typing and st.session_state.get("last_ai_audio"):
        ...
        terminal_content = st.session_state.terminal_log.replace('<', '&lt;').replace('>', '&gt;').replace('\n', '<br>')
        
        # Use streamlit components for proper JavaScript execution
        components.html(f"""
        <div style="background: #000000; color: #90EE90; font-family: 'Courier New', monospace; padding: 20px; border: 1px solid #90EE90; border-radius: 5px; height: 400px; overflow-y: auto;" id="terminal">
            {terminal_content}<span id="ai-typing"></span><span id="cursor" style="animation: blink 1s infinite; color: #90EE90;">|</span>
        </div>
...
def create_audio_button(audio_data, button_text, audio_format="wav"):
    """Create a simple play button for audio"""
    import base64
    import streamlit.components.v1 as components
    import random
    
    if not audio_data:
        st.info(f"No audio available")
        return
    ...
    unique_id = f"audio_{random.randint(1000, 9999)}"
    button_id = f"btn_{unique_id}"
    
    # Create HTML with button and hidden audio
    html_code = f"""
    <div style="margin: 10px 0;">
        <button id="{button_id}" onclick="playAudio_{unique_id}()" style="
            background: #006400; 
            border: 1px solid #90EE90; 
            color: #90EE90; 
            padding: 12px 20px; 
        ...
        <audio id="{unique_id}" preload="auto" style="display: none;">
            <source src="data:{mime_type};base64,{audio_b64}" type="{mime_type}">
        </audio>
    </div>
    
    <script>
        function playAudio_{unique_id}() {{
            ...
            if (audio.paused) {{
                audio.play().then(() => {{
                    ...
                    button.style.background = '#004d00';
                }}).catch(e => {{
                    button.textContent = '❌ Play Failed';
                    button.style.background = '#640000';
                    setTimeout(() => {{
...
with col1:
    st.markdown("### Main Terminal - Sync Mode")
    
    # Use terminal with sync capability
    create_terminal_with_sync()
    
    # Add basic audio players below terminal
    st.markdown("---")
    st.markdown("### Manual Audio Players")
    ...
    with audio_col1:
        ...
        if st.session_state.get('current_input_audio'):
            ...
            # Use button instead of audio bar
            create_audio_button(
                st.session_state.current_input_audio, 
                "🎵 PLAY YOUR VOICE", 
                "wav"
            )
        else:
        ...
        if st.button("START EMPATHETIC CONVERSATION", type="primary", use_container_width=True):
            ...
            if os.path.exists(audio_path):
                ...
                with st.spinner("Understanding your emotions"):
                    ...
                    # Add AI response with typing effect enabled
                    add_to_terminal(clean_ai_response, "assistant", enable_typing=True)
                    ...
                    st.success("Conversation complete - Watch terminal for sync effect!")
                    st.rerun()
...
3. Watch terminal for automatic typing effect with synchronized audio
4. Use manual audio players below for additional playback
5. All text is cleaned for robot compatibility (no punctuation or emojis)
6. Green terminal displays conversations with sync effects

**SYNC FEATURES:**
- AI responses automatically play with typing effect
- Audio and text are perfectly synchronized
- Manual players available for additional control

**Supported formats:** WAV MP3 OGG M4A FLAC
**Robot compatible:** No punctuation, no emojis, clean text only
**Sync mode:** Automatic typing effect with audio playback
""")

# Live status - ROBOT READY
st.markdown('<p class="status-loading">EMPATHY ENGINE ACTIVE - SYNC MODE READY</p>', unsafe_allow_html=True)